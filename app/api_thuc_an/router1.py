...
# Load models
model_class = joblib.load("app/api_thuc_an/model_forest_class.pkl")
model_sang = joblib.load("app/api_thuc_an/model_forest_sang.pkl")
model_chieu = joblib.load("app/api_thuc_an/model_forest_chieu.pkl")
model_2buoi = joblib.load("app/api_thuc_an/model_forest_2buoi.pkl")
...
model_soluongca = joblib.load("app/api_thuc_an/model_soluongca.pkl")
model_matdo = joblib.load("app/api_thuc_an/model_matdo.pkl")
model_sanluongca = joblib.load("app/api_thuc_an/model_sanluongca.pkl")
...
thuoc_list = [f for f in base_feature_names if f not in
              ['size', 'sanluongca', 'tuoi', 'tile_hao hut', 'matdo', 'soluongca', 'loai_xl',
               'ck', 'gtm', 'tgtm', 'xh', 'xx']]
...
def predict_feed(data: FishFeedInput):
    ...
    if not data.so_ngay_du_doan or data.so_ngay_du_doan <= 1:
        ...
        if loai_cho_an == 1:
            ...
            food_sang, food_chieu = model_2buoi.predict(x_2buoi)[0]
            result.update({
