...
# Load models
model_class = joblib.load("app/api_thuc_an/thuoc_exclude/model_lgbm_class.pkl")
model_sang = joblib.load("app/api_thuc_an/thuoc_exclude/model_xgb_sang.pkl")
model_chieu = joblib.load("app/api_thuc_an/thuoc_exclude/model_forest_chieu_new.pkl")
model_2buoi = joblib.load("app/api_thuc_an/thuoc_exclude/model_xgb_multi.pkl")
model_loai_thuc_an = joblib.load("app/api_thuc_an/thuoc_exclude/model_forest_loai_thuc_an.pkl")
...
def predict_feed(data: FishFeedInput):
    ...
    def build_input_vector(model, state, extra={}):
        vector = []
    ...
    if not data.so_ngay_du_doan or data.so_ngay_du_doan <= 1:
        ...
        feature_vector = build_input_vector(model_class, state)
        ...
        try:
            ...
            result["loai_thuc_an"] = str(predicted_label)
        except Exception as e:
            print("LỖI DỰ ĐOÁN LOẠI THỨC ĂN:", e)
            traceback.print_exc()
            result["loai_thuc_an"] = "Không xác định"
            result["debug_loai_thuc_an"] = str(e)
            result["traceback"] = traceback.format_exc()

        # Dự đoán lượng cho ăn
        if loai_cho_an == 0:
            x_2buoi = build_input_vector(model_2buoi, state)
            ...
            x_sang = build_input_vector(model_sang, state)
            food = float(model_sang.predict(x_sang)[0])
            result["sang"] = round(food, 2)
        elif loai_cho_an == 2:
            x_chieu = build_input_vector(model_chieu, state)
            food = float(model_chieu.predict(x_chieu)[0])
            result["chieu"] = round(food, 2)

        # ÉP KIỂU TẤT CẢ TRƯỜNG TRƯỚC KHI TRẢ VỀ
    ...
    # 3. Size cá không giảm so với hôm qua
        ...
        if "model_size" in globals():
            x_size = build_input_vector(model_size, state)
    ...
    # 6. Dự đoán loại cho ăn, loại thức ăn, lượng thức ăn...
        feature_vector = build_input_vector(
            model_class,
            {**state, "size_interp": next_size, "soluongca": next_soluongca,'size_giong':data.size_giong, "matdo": next_matdo, "sanluongca": next_sanluongca, "tile_hao_hut": tile_hao_hut_luy_ke}
        ...
        try:
            ...
            # In ra server log (để thấy trace trên console/server)
            print("LỖI DỰ ĐOÁN LOẠI THỨC ĂN:", e)
            traceback.print_exc()
            # Trả về message lỗi cho phía frontend (chỉ DEV mới nên dùng)
        ...
        du_doan_nhieu_ngay.append({
            "ngay": int(day + 1),
            "tuoi": int(state['tuoi'] + 1),
            "size_interp": float(round(next_size, 2)),
            "soluongca": int(next_soluongca),
            "matdo": float(round(next_matdo, 2)),
            "sanluongca": float(round(next_sanluongca, 2)),
            "tile_hao_hut": float(round(tile_hao_hut_luy_ke, 2)),
            "loai_cho_an": int(loai_cho_an),
            "loai_thuc_an": str(predicted_label),
            **{k: float(v) if isinstance(v, (int, float, np.integer, np.floating)) else v for k, v in feed_info.items()}
        })
        ...
        state['size_giong'] = data.size_giong
